...
void ssd1306_draw_char(char c, uint8_t x, uint8_t y)
{
  ...
  if (c >= 'A' && c <= 'Z')
  {
    index = (c - 'A' + 1) * 8; // Para letras maiúsculas
  }
  else if (c >= 'a' && c <= 'z')
  {
    index = (c - 'a' + 27) * 8; // Para letras minúsculas
  }
  if (c >= '0' && c <= '9')
  {
    index = (c - '0' + 53) * 8; // Para números
  }
  else if (c == '\'')
  {
    index = 63 * 8; // Para o caractere "'"
  }
  else if (c == ':')
  {
    index = 64 * 8; // Para o caractere ":"
  }
